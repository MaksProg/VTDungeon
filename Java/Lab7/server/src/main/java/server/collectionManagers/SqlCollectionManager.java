...
public class SqlCollectionManager implements CollectionManager {
    private static final String CREATE_TABLE_QUERY = "CREATE TABLE IF NOT EXISTS venues ("
    ...
    public void initTables() throws SQLException{
        ...
        try (PreparedStatement ps = conn.prepareStatement(SELECT_TICKETS_QUERY);
            ...
            logger.info("Loaded " + collection.size() + " tickets, removed " + invalidTickets + " invalid tickets.");
        }
    }
    ...
    public static void prepareVenueStatement(PreparedStatement s, Venue venue, int paramOffset) throws SQLException {
    ...
    public static void prepareTicketStatement(PreparedStatement s, Ticket ticket, int paramOffset) throws SQLException {
    ...
    private Integer findVenueId(Venue venue) throws SQLException {
        String query = "SELECT id FROM venues WHERE name = ? AND capacity = ? AND type = ? AND zip_code = ? AND x = ? AND y = ? AND town_name = ?";
    ...
    public int clearCollection(String username) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(deleteSQL)) {
            ...
            logger.warning("Ошибка при удалении билетов пользователя '" + username + "': " + e.getMessage());
        }
    ...
    public void addTicket(Ticket ticket) throws SQLException {
        ...
        String insertTicketSQL = "INSERT INTO tickets " +
    ...
    private int insertOrGetVenueId(Venue venue) throws SQLException {
        ...
        String insertVenueSQL = "INSERT INTO venues (name, capacity, type, zip_code, x, y, town_name) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING id";
    ...
    """;
        ...
        try (PreparedStatement checkStmt = conn.prepareStatement(checkOwnershipQuery)) {
            ...
            if (venue != null) {
                ...
                if ((venue.getName() == null || venue.getType() == null) && venue.getId() != null && venue.getId() != 0) {
                    Venue fullVenue = getVenueById(venue.getId());
                    if (fullVenue == null) {
                        logger.warning("Площадка с id " + venue.getId() + " не найдена.");
                        return false;
